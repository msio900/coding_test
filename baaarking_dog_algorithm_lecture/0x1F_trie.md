...
| 자식 저장 방법     | 코드                                  | 시간복잡도       | 공간복잡도 | 시간 <br />*백준 14425 기준 | 메모리<br />*백준 14425 기준 |
| ------------------ | ------------------------------------- | ---------------- | ---------- | --------------------------- | ---------------------------- |
| 고정된 크기의 배열 | `nxt[MX][26]`                         | `O(|S|)`         | `O(26*MX)` | 472ms                       | 514ms                        |
| 동적 배열          | verctor<pair<char, int>><br />nxt[MX] | `O(25*|S|)`      | `O[MX]`    | 2018ms                      | 280ms                        |
| 연결 리스트        | list<pair<char, int>><br />nxt[MX]    | `O(26*|S|)`      | `O[MX]`    | 3732ms                      | 280ms                        |
| 이진 검색 트리     | map<char, int> nxt[MX]                | `O(log(26)*|S|)` | `O[MX]`    | 688ms                       | 474ms                        |
| 해시               | unordered_map<char, int><br />nxt[MX] | `O(|S|)`         | `O[MX]`    | 1584ms                      | 979ms                        |
